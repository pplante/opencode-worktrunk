...
- [opencode](https://opencode.ai) 1.17+ (v1 file install) or 2.x (v2 package install)
...
| `src/worktrunk-wt.ts` | V1 server plugin entry — registers 5 tools and hooks (`permission.ask`, `shell.env`, `tool.execute.before`, message bootstrap) |
| `src/server.ts` | V2 server plugin entry (`Plugin.define`, id `worktrunk`) — same 5 tools plus `wt` aliases, `permission.evaluate` / `tool.execute.before` / `session.context` hooks, `session.deleted` cleanup |
